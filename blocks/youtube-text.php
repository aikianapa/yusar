        ...
        <div class="row software-service--content">
            <div class="col-lg-6 col-md-12 {{pos}}">
                <div style="border-radius: 10px; height: 420px; overflow: hidden;">
                    <iframe wb-if="'{{youtube.{{_sess.lang}}}}'>''" src="{{urldecode({{youtube.{{_sess.lang}}}})}}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
            </div>
            <div class="col-lg-6 col-md-12 service-text text-break">{{text.{{_sess.lang}}}}</div>
        </div>
        <div class="software-service--info text-break" wb-if="'{{text_bottom.{{_sess.lang}}}}'>''">{{text_bottom.{{_sess.lang}}}}</div>
    </div>
</view>